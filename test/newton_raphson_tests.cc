...
struct NewtonRaphsonData
{
  ...
  inline friend std::ostream& operator<<(std::ostream& out,
                                         NewtonRaphsonData const& data)
  {
    out << "{";
    out << data.lowerBound << "," << data.upperBound << "," << data.roundLimit
        << "," << data.initialU << ",";
...
TEST_P(NewtonRaphsonTests, findroot_correctResult)
{
  ...
  auto const result = NewtonRaphson::findRoot(params.lowerBound,
                                              params.initialU,
                                              params.upperBound,
                                              params.roundLimit,
                                              params.f,
...
auto const parabel = [](real const u) -> std::optional<real> {
  return u * u - real(1);
};
auto const d_parabel = [](real const u) -> std::optional<real> {
  return real(2) * u;
};
...
INSTANTIATE_TEST_SUITE_P(Fixture,
                         NewtonRaphsonTests,
                         ::testing::ValuesIn(newtonRaphsonData));
