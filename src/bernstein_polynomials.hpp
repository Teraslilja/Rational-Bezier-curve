...
protected:
   ...
   *  @return 0 otherwise
   */
  [[nodiscard]] static constexpr std::size_t binomialRecursiveMult(
    std::size_t const n,
    std::size_t const k) noexcept
  {
    auto constexpr bin = [](std::size_t const n,
                            std::size_t const k) constexpr -> std::size_t {
      auto constexpr impl = [](auto&& self,
                               std::size_t const n,
                               std::size_t const k) constexpr -> std::size_t {
        switch (k) {
          case 0u:
   ...
   *  @return 0 otherwise
   */
  [[nodiscard]] static std::size_t binomialRecursiveSum(
    std::size_t const n,
    std::size_t const k) noexcept
  {
    auto constexpr bin = [](std::size_t const n,
                            std::size_t const k) -> std::size_t {
      auto constexpr impl = [](auto const& self,
                               std::size_t const n,
                               std::size_t const k) -> std::size_t {
        switch (k) {
          case 0u:
  ...
  [[nodiscard]] static constexpr std::size_t binomialFallingFactorial(
    ...
    std::size_t const k) noexcept
  {
    auto constexpr impl = [](std::size_t const n,
                             std::size_t const k) constexpr -> std::size_t {
   ...
   *  @return 0 otherwise
   */
  [[nodiscard]] static constexpr std::size_t binomialInPlace(
    std::size_t const n,
    std::size_t k)
  {
    auto constexpr impl = [](std::size_t const n,
      ...
      if ((n == 0) || (k == 0)) {
        ...
        std::size_t* const column = reinterpret_cast<std::size_t*>(
          alloca(sizeof(std::size_t) * (n << 1u)));
  ...
  /**
   *  @brief Calculate binomial coeffient
   * \f$\binom{n}{k}=\frac{n!}{k!\left(n-k\right)!}\f$ with the fastest
   * implementation
   *
   *  @param n "k objects can be chosen from among n objects", n >= k >= 0
   *  @param k "k objects can be chosen from among n objects", n >= k >= 0
   ...
   *  @return 0 otherwise
   */
  [[nodiscard]] static constexpr std::size_t binomial(
    std::size_t const n,
    std::size_t const k) noexcept
  {
...
public:
                                        ...
                                        real const u) noexcept
  {
    ...
    real const b = real(binomial(n, i)) * toIntegerPower(u, i) *
