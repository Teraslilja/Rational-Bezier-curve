...
/**
 *  @file control_point.hpp This file contains module interface for control
 * point
 */
...
/**
 *  @brief Control point class for rational bezier curves. Uses either @ref
 * curve::points::Point2 or @ref curve::points::Point3
 */
...
public:
   ...
   *  @param p the point
   */
  constexpr ControlPoint(real const w, Point const p) noexcept
    : w_(w)
    , p_(p)
  {
  }
  ...
  /**
   *  @brief Return reference to (coordinate) values of control point for
   * modifying purposes
   *
   ...
   *
   *  @param out output stream
   *  @param data optional control point or if std::nullopt output string "'no
   * value'"
   *  @return 'out' stream
   */
  friend std::ostream& operator<<(std::ostream& out,
                                  std::optional<Controlpoint> const& data)
  {
    if (data.has_value()) {
      out << data.value();
    } else {
   ...
   */
  friend std::ostream& operator<<(std::ostream& out,
                                  std::vector<Controlpoint> const& data)
  {
    out << "[";
