...
/**
 *  @file rational_bezier.hpp This file contains module interface for rational
 * bezier
 */
...
/**
 *  @brief The class for rational bezier curves with management of control
 * points of type 'CP', either Point2<> or Point3<>. Delegates calls to @ref
 * curve::bezier::rational::ValidateRational
 */
template<class CP>
...
public:
  ...
  constexpr Rational(ControlPointContainer const& cp)
  {
    ...
    this->controlPoints_.insert(
      this->controlPoints_.begin(), cp.cbegin(), cp.cend());
  }
   ...
   */
  constexpr Rational(ControlPointContainer&& cp) noexcept
  {
    this->controlPoints_ = std::move(cp);
  }
  ...
  {
    this->controlPoints_ = ControlPointContainer();
  }
...
public:
   ...
   *  @return ValidityIssue as std::variant, if any problem with curve
   */
  [[nodiscard]] inline point_or_issue closestCurvePointFor(
    ConstPoint p) const noexcept
  {
    Validator const validator(this->controlPointContainerAsSpan());
  ...
  /**
   *  @brief Get a reference to control point at index 'index' for modifying
   * purposes. NOTE1: this reference may get invalidated, if allocation of
   * container of control points is changed. NOTE2: If weight is modified so
   * that sum of all weights goes near zero, the curve shall be invalid
   *
   *  @param index the index of control point, valid range [ 0 ; \<number of
   * control points\> [
  ...
  [[nodiscard]] constexpr std::optional<
    std::reference_wrapper<ControlPoint>> const
  ...
  /**
   *  @brief Get a const reference to control point at index 'index' for reading
   * purposes NOTE: this reference may get invalidated, if allocation of
   * container of control points is changed.
   *
   *  @param index the index of control point, valid range [ 0 ; \<number of
   * control points> \[
   ...
   *  @return true, if the index is within range [ 0 ; \<number of control
   * points\> [
   *  @return false otherwise
   */
  [[nodiscard]] constexpr bool addControlPoint(std::size_t const index,
                                               ControlPoint const cp) noexcept
  {
    if (index <= this->numberOfControlPoints()) {
      try {
        (void)this->controlPoints_.emplace(this->controlPoints_.begin() + index,
                                           cp); // Might throw
      }
   ...
   *  @return false otherwise
   */
  [[nodiscard]] constexpr bool removeControlPoint(
  ...
  /**
   *  @brief Remove all control points from curve, allocation of container for
   * control points is not changed.
   */
  constexpr void removeAllControlPoints() noexcept
  {
   ...
   *
   *  @return Number of control points that can be stored to the container
   * without increasing allocation for container
   */
  constexpr std::size_t capacity() const noexcept
  {
    return this->controlPoints_.capacity();
  }

  /**
   *  @brief Reserve 'n' number of control points at container. Allocation is
   * changed, if 'n' is greater than the current capacity
   *
  ...
  /**
   *  @brief Match capacity of container of control points with the number of
   * control points at container
   */
...
protected:
   ...
   */
  [[nodiscard]] constexpr ControlPointSpan controlPointContainerAsSpan()
    const noexcept
  {
    return ControlPointSpan(this->controlPoints_.cbegin(),
                            this->controlPoints_.size());
  }
